...
void AdasDataModel::updateData()
{
    ...
    if (m_count > MAX_TYPE*MAX_RANGE_LEVEL) {
        m_count = 1;
    }
    qDebug() << "AdasDataModel updateData m_count = " << m_count;
...
{
    qDebug() << "AdasDataModel: adas_obstacle_on = " << !m_wait;
    return !m_wait;
}

DATA_PROPERTY_IMPLEMENT(AdasDataModel, adas_obstacle_x, int)
{
    qDebug() << "AdasDataModel: adas_obstacle_x = " << obstacles[m_count].x;
    return obstacles[m_count].x;
}

DATA_PROPERTY_IMPLEMENT(AdasDataModel, adas_obstacle_y, int)
{
    qDebug() << "AdasDataModel: adas_obstacle_y = " << obstacles[m_count].y;
    return obstacles[m_count].y;
}

DATA_PROPERTY_IMPLEMENT(AdasDataModel, adas_obstacle_image, QString)
{
    qDebug() << "AdasDataModel: adas_obstacle_image = " << QString(obstacles[m_count].url);
    return QString(obstacles[m_count].url);
}
